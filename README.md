...
If you're running an NDI® target, you may encounter problems in receive the video stream. This can be easily solved by instead running the project as root. If you still have problems and you're under WSL, it's probably because its NIC is natted from your lan. To put the wsl's nic in bridge mode you have to: Open hyper-v manager as administrator, go on virtual switch manager, choose wsl, under connection type put `external network`, choose the correct NIC and apply. It may gave you an error, close the virtual switch manager, repoen it, put the same settings as before, reapply and this time it should work. Now open wsl and write the following commands:
```
ip addr add  <the ip you wish for your wsl>/<your subnet mask in decimal notation> dev eth0
ip route del <your network's ip>/<subnet mask>
ip route del default
ip route add <your gateway's ip> dev eth0
ip route add default via <your gateway's ip>
echo "nameserver 1.0.0.1" > /etc/resolv.conf
echo "nameserver 8.8.8.8" >> /etc/resolv.conf
```
Note: This configuration will survive windows' shutdowns (you may need to re-enter the previous commands every time tho), however it will not survive full reboots. When you fully reboot your windows OS you'll probably end up with a broken NIC, even in the host. To fix that and make it work again, you must: open control panel, go to network connections, open your NIC's settings, proprieties, and disable `Hyper-V Extensible Virtual Switch`. Apply everything and you should now be ok. You may also need to reset your IPv4 settings (like static local ip, DNSs, etc). And yeah, you also have to redo the previous procedure if you want to put again your WSL in your same LAN, without any kind of NAT
...
To compile the version that simply renders a static image and outputs the execution time, you just need to run `make llf` (for the single core version), `make openmp` (for the multithread version), `make cuda` (for the gpu version). The binaries will be placed under the `bin/` folder. Since they output the rendered image to stdout, the best way to run them is by using the script `scripts/testProgram.sh <llf | openmp | cuda>` or by commenting the `printStaticImage4(img4);` line in [src/llf/mainLlf.cpp](src/llf/mainLlf.cpp), or [src/OpenMP/openmpMain.cpp](src/OpenMP/openmpMain.cpp), or [src/CUDA/mainCuda.cpp](src/CUDA/mainCuda.cpp). You can change the static image it's going to render by editing the testimage of the [makefile in the project's root](makefile#L53). If you want to edit the llf's parameters, you have to change the pars of the llf call in the previous main files.

The openmp and cuda executables take some extra parameters. In openmp you have to specify the number of threads, in cuda you have to specify the number of blocks and number of threads for each block. The "testStaticImage" script comes with some default parameters we used while testing (24 threads for openmp, 512 blocks and 256 threads for cuda), you have to edit [scripts/testProgram.sh](scripts/testProgram.sh) to change them

The output rendered image will be placed inside the `tmp/` directory

## Real time rendering with NDI®

To compile, instead, the version that will take an NDI® stream and renders it in real time, you need to run the target `make real-rime-llf` (for the single core version), `make real-rime-openmp` (for the multithread version), `make real-rime-cuda` (for the gpu version). The binaries will be placed under the `bin/` folder. You can directly run them, however they take some argouments:
```
